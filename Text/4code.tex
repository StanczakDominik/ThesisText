...
\begin{itemize}
    ...
    \item \code{BC} - \english{Boundary Condition}, obiekt zawierający informacje dotyczące
...
\begin{itemize}
     ...
     \item \code{gather\_current} - zbiera ładunek z cząstek na siatkę
     \item \code{gather\_charge} - zbiera prąd z cząstek na siatkę
     \item \code{init\_solve} - uruchamia początkową, spektralną iterację obliczającą równowagowe
           pole elektrostatyczne na podstawie gęstości ładunku z cząstek
     \item \code{solve} - uruchamia iterację ewolucji czasowej pola elektromagnetycznego na podstawie
           prądów w symulacji
     \item \code{field\_function} - interpoluje pola elektryczne i magnetyczne do danych położeń, używane
           do obliczenia pól w cząstkach
     \item \code{save\_field\_values} - zapisuje dane dotyczące wartości historycznych na siatce w danej iteracji.
           Należy zaznaczyć, że wielkości na komórkach granicznych nie są zapisywane.
     \item \code{postprocess} - przetwarza zebrane w czasie symulacji minimalne informacje dotyczące pól, prądów i ładunków
...
\begin{itemize}
    \item \code{apply\_bc} - aplikuje warunki brzegowe. W symulacjach okresowych jest to przywrócenie cząstek do rejonu symulacji ($x := x \% L$), w symulacjach nieokresowych usunięcie ich.
    \item \code{position\_push}  - przemieszcza cząstki w przestrzeni.
    \item \code{velocity\_push} - interpoluje pola do pozycji cząstek i aktualizuje ich prędkości na podstawie integratora Borysa.
    \item \code{save\_particle\_values} - zapisuje wartości danych historycznych cząstek.
    \item \code{distribute\_uniformly} - rozmieszcza cząstki równomiernie na zadanym przedziale
    \item \code{distribute\_nonuniformly} - rozmieszcza cząstki według zadanej funkcji rozkładu gęstości
\end{itemize}
...
\begin{itemize}
    \item \code{BoundaryCondition} - implementacja warunków brzegowych
    \item \code{charge\_deposition} - depozycja ładunku na siatkę
    \item \code{current\_deposition} - depozycja prądu podłużnego i poprzecznego na siatkę
    \item \code{density\_profiles} - profile przestrzenne kształtów preplazmy
        oraz algorytmy rozkładające cząstki według tych rozkładów.
    \item \code{FieldSolver} - spektralne i rotacyjne algorytmy rozwiązywania równań Maxwella na siatce Eulera.
    \item \code{particle\_push} - implementacja integratorów równań ruchu typu \english{leapfrog} oraz Borysa.
    \item \code{field\_interpolation} - interpolacja pól elektromagnetycznych z położeń na siatce do cząstek.
\end{itemize}

Katalog \code{visualization} zawiera wizualizacje danych.
\begin{itemize}
    \item \code{animation} - tworzy animacje dla celów analizy danych
    \item \code{time\_snapshots} - implementacje wykresów zależnych od czasu, ilustrujących stan symulacji w danej iteracji.
    \item \code{static\_plots} - tworzy statyczne wykresy dla celów analizy danych
    \item \code{plotting} - zawiera ustawienia dotyczące analizy danych
\end{itemize}
...
\begin{figure}[h!]
  ...
  \caption{Wizualizacja szybkości działania poszczególnych fragmentów kodu
    wygenerowana programem \code{snakeviz}.}
  \label{fig:snakeviz}
\end{figure}
