\section[Opis algorytmów PIC]{Opis algorytmów PIC}% 20-30% - opis przyjętych rozwiązań i uzasadnienie ich wyboru
...
przez łącznie trzy komórki. \todo[inline]{zweryfikować z algorytmem} % TODO005
...
\subsubsection{Depozycja podłużna} % TODO
\begin{enumerate}
    ...
    %TODO006 jaki współczynnik do której komórki?
\end{enumerate}
...
\subsubsection{Depozycja poprzeczna} % TODO
...
\begin{equation}
    s =
\end{equation}
% TODO007 wzór na przekrycie z instrukcji J.
...
\begin{equation}
    w = {(s(x^{n+1}) + s(x^n))} % TODO008 wzór na przekrycie z instrukcji J
\end{equation}
...
\subsubsection{Warunek początkowy pola elektrycznego: solver globalny, elektrostatyczny}
Za \cite{birdsall}, wychodzimy z równania Poissona:
% TODO004

\subsubsection{Solver lokalny}
Wychodzimy z rotacyjnych równań Maxwella\ref{eqn:maxwell-E-rotation}\ref{eqn:maxwell-B-rotation}.
...
% TODO009 Finish derivation of transversal solver
\begin{equation}
...
\begin{equation}
    {F^{+}}^{n+1}_{i+1} = F^{+}_{n} + j
\end{equation}
% TODO010 finish derivation of longitudinal solver
